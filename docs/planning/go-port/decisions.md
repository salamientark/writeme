...
| G1 | V1 scope confirm | **Phases 1, 2, 3, 6 only.** Phases 4 & 5 (TUI) ship in v1.1. | Plain-mode UI is the only UI surface in v1. `internal/selection` stays render-free so v1.1 wraps it in `tea.Model.Update` with no refactor. |
| G2 | SIGINT model | **`signal.NotifyContext(ctx, os.Interrupt, syscall.SIGTERM)`** at top of `main`. Ctx threaded through fetch, workers, subprocesses. `defer` summary print + `os.Exit(130)` when `ctx.Err() == context.Canceled`. | Single code path. `exec.CommandContext` kills children on cancel. State writes already mutex-guarded and synchronous — no separate flush step. |
...
| G4 | Fake binaries | **Single Go test helper binary** built via `TestMain`; dispatches on `filepath.Base(os.Args[0])` to fake `gh` / fake `claude`. Fixtures driven by env vars (`WRITEME_FAKE_GH_FIXTURE`, `WRITEME_FAKE_CLAUDE_OUTCOME`). Tests symlink into tmp PATH dir. | Replaces `testdata/fakegh.sh` (task-list.md Phase 2 task 5). Pure Go, debuggable, captures argv to log file for assertions. |
| G5 | Subprocess kill | **`SysProcAttr{Setpgid: true}`** + `syscall.Kill(-pid, SIGKILL)` on cancel. Build-tagged `subproc_unix.go` (`//go:build linux \|\| darwin`). | Reaps `claude` + child node/MCP procs. V1 platforms = linux/darwin only — Windows excluded from release matrix. |
| G6 | Plain-mode selection | **Numbered list + range expression.** Prompt: `Select repos (e.g. 1,3-5,8 or 'all'): `. Range parser ported from `tests/test_range_parser.py`. | Stateless, scriptable, CI-friendly. Empty input = `Nothing selected.` and exit 0 (matches Python). |
| G7 | Plain-mode review serialization | **One consumer goroutine** drains result chan (size = `len(jobs)`); per result, prints diff + `[a]/[r]/[d]/[q]` prompt; blocks on stdin until valid keypress. Workers continue producing in parallel. | Strict serial UI; throughput preserved by buffered chan. Matches Python single-threaded UI. |
| G8 | E2E parity check | **Pre-recorded Python goldens only**, captured once in Phase 0 against `v0-python-final` tag. Go e2e diffs against committed fixtures. | No Python on CI. Re-record only on spec change. Matches D3, D6. |
| G9 | Coverage gate | **Per-package gate** enforced in CI: any `go/internal/*` package <80% line coverage fails build. `cmd/writeme` excluded. | Phase exit criteria are per-package; aggregate-only gate would let weak packages hide. Awk script over `go test -coverprofile=cover.out ./...`. |
| G10 | Sandbox cleanup | **Always remove per-job XDG** (`<repos-dir>/.sandbox/claude-jobs/<repo>/`) via `defer` regardless of outcome. Repo clone preserved across runs. Failure log written **before** cleanup. | `NUKE_ON_FAIL` is launcher-only per `spec.md` §5 — pipeline does not implement it. |
| G11 | Phase ordering | **Strict sequential 1 → 2 → 3 → 6.** No interleaving. | Each phase has parity checkpoint, exit-criteria gate, TDD loop (test → impl → `/go-test` → `/go-build` → `/go-review`). Clean review boundaries. |
